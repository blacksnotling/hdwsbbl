...
<?php get_header(); ?>
	<?php if (have_posts()) : ?>
		...
		<div id="breadcrumb">
			<p><a href="<?php echo get_option('home'); ?>" title="Back to the front of the BBBBL">BBBBL</a> &raquo; <a href="<?php echo get_option('home'); ?>/stats" title="Back to the main Statistics page">Statistics</a> &raquo; <?php the_title(); ?></p>
		</div>
			<div class="entry">
				<h2><?php the_title(); ?></h2>
...
?>
		...
		<ul>
			...
			<li><strong>Average Career length of a BBBBL Player</strong>: <?php print(round($matchrec/$playernum,1)); ?> games</li>
		</ul>
...
<?php
				...
				if ($topstats = $wpdb->get_results($statsql)) {
					if ($period_alltime) {
						print("	<p>Players who are <strong>highlighted</strong> are still active in the BBBBL.</p>\n");
					}
