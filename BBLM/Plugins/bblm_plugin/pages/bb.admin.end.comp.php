...
?>
	...
	<p>Please slelect the competition you wish to close.</p>
		...
		<?php
		$compsql = 'SELECT C.c_id, C.c_name, C.series_id, S.sea_name FROM '.$wpdb->prefix.'comp C, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' P, '.$wpdb->prefix.'season S WHERE S.sea_id = C.sea_id AND C.c_id = J.tid AND J.prefix = \'c_\' AND J.pid = P.ID AND C.c_active = 1';
		if ($comps = $wpdb->get_results($compsql)) {
			foreach ($comps as $comp) {
				print("<option value=\"".$comp->c_id."\">".$comp->c_name." - ".$comp->sea_name." - ". bblm_get_cup_name( $comp->series_id ) ."</option>\n");
			}
		}
		?>
	</select>
