...
<div class="wrap">
<?php
	echo '<h2>' . __( 'Edit Fixtures', 'bblm' ) . '</h2>';
	echo '<p>' . __( 'Below is a list of all fixtures that have not been played yet. Ammend the details and tick the "changed" box to update one.', 'bblm' ) . '</p>';
...
<?php
	...
	if ( 13 == $_POST['bblm_fdiv'] ) {
		$teamssql = 'SELECT P.post_title, C.t_id FROM '.$wpdb->prefix.'team_comp C, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' P WHERE C.t_id = J.tid AND J.prefix = \'t_\' AND J.pid = P.ID AND C.c_id = '.intval( $_POST['bblm_fcomp'] ).' ORDER BY P.post_title ASC';
	}
	else {
...
<?php
	...
	if ($fixtures = $wpdb->get_results($fixturessql)) {
		...
		print("<input type=\"hidden\" name=\"bblm_fcomp\" size=\"2\" value=\"".intval( $_POST['bblm_fcomp'] )."\">\n");
		print("<input type=\"hidden\" name=\"bblm_fdiv\" size=\"2\" value=\"".intval( $_POST['bblm_fdiv'] )."\">\n");
	}
	else {
