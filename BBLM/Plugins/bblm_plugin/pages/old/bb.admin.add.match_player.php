...
while ($p <= $pmax){
	...
	if (on == $_POST['bblm_pcng'.$p]) {
		...
		$updatesql = 'UPDATE `bb_player` SET `p_ma` = \''.$_POST['bblm_pma'.$p].'\', `p_st` = \''.$_POST['bblm_pst'.$p].'\', `p_ag` = \''.$_POST['bblm_pag'.$p].'\', `p_av` = \''.$_POST['bblm_pav'.$p].'\', `p_skills` = \''.$_POST['bblm_pskills'.$p].'\', `p_injuries` = \''.$_POST['bblm_pinjuries'.$p].'\', `p_cost` = \''.$_POST['bblm_pcost'.$p].'\'';
...
$updatematchsql = 'UPDATE `bb_match` SET `m_complete` = \'1\' WHERE `m_id` = '.$_POST['bblm_mid'].' LIMIT 1';
...
$playerAsql = 'SELECT SUM(p_cost_ng) AS cost FROM bb_player WHERE p_status = 1 AND p_mng = 0 AND t_id = '.$_POST['bblm_teamA'];
	$teamA_pc = $wpdb->get_var($playerAsql);
$teamAsql = 'SELECT SUM((R.r_rrcost*T.t_rr) + (T.t_ff+T.t_apoc+T.t_cl+T.t_ac)*10000) AS SUM FROM bb_team T, bb_race R WHERE T.r_id = R.r_id AND T.t_id = '.$_POST['bblm_teamA'];
	$teamA_tc = $wpdb->get_var($teamAsql);
	$teamA_tv = $teamA_pc + $teamA_tc;

$updateteamAsql = 'UPDATE `bb_team` SET `t_tv` = \''.$teamA_tv.'\' WHERE `t_id` = '.$_POST['bblm_teamA'].' LIMIT 1';

$playerBsql = 'SELECT SUM(p_cost_ng) AS cost FROM bb_player WHERE p_status = 1 AND p_mng = 0 AND t_id = '.$_POST['bblm_teamB'];
	$teamB_pc = $wpdb->get_var($playerBsql);
$teamBsql = 'SELECT SUM((R.r_rrcost*T.t_rr) + (T.t_ff+T.t_apoc+T.t_cl+T.t_ac)*10000) AS SUM FROM bb_team T, bb_race R WHERE T.r_id = R.r_id AND T.t_id = '.$_POST['bblm_teamB'];
	$teamB_tc = $wpdb->get_var($teamBsql);
	$teamB_tv = $teamB_pc + $teamB_tc;

$updateteamBsql = 'UPDATE `bb_team` SET `t_tv` = \''.$teamB_tv.'\' WHERE `t_id` = '.$_POST['bblm_teamB'].' LIMIT 1';
*/
...
$playermatchsql = "INSERT INTO `bb_match_player` (`m_id`, `p_id`, `t_id`, `mp_td`, `mp_cas`, `mp_comp`, `mp_int`, `mp_mvp`, `mp_spp`, `mp_mng`, `mp_inj`, `mp_inc`) VALUES ";
	...
	if ($compcounts)  {
		$selectinjplayer = 'SELECT p_id FROM bb_player WHERE p_mng = 1 AND (t_id = '.$_POST['bblm_teamA'].' or t_id = '.$_POST['bblm_teamB'].')';

		if ($injplayer = $wpdb->get_results($selectinjplayer)) {
			foreach ($injplayer as $ip) {
				$reactivatesql[] .= 'UPDATE `bb_player` SET `p_mng` = \'0\', `p_cost_ng` = p_cost  WHERE `p_id` = '.$ip->p_id.' LIMIT 1';
			}
		}
...
if (on == $_POST['bblm_plyd'.$p]) {
	...
	//generate the SQL
	$playermatchsql .= '(\''.$_POST['bblm_mid'].'\', \''.$_POST['bblm_pid'.$p].'\', \''.$_POST['bblm_tid'.$p].'\', \''.$_POST['bblm_td'.$p].'\', \''.$_POST['bblm_cas'.$p].'\', \''.$_POST['bblm_comp'.$p].'\', \''.$_POST['bblm_int'.$p].'\', \''.$_POST['bblm_mvp'.$p].'\', \''.$_POST['bblm_spp'.$p].'\', \''.$_POST['mng'.$p].'\', \''.$_POST['bblm_injury'.$p].'\', \''.$_POST['bblm_increase'.$p].'\')';

	//if the comp counts, update the bb_player table with new spp values and p_mng if player is injured.
	if ($compcounts)  {
		$playerupdatesql = 'UPDATE `bb_player` SET `p_spp` = p_spp+\''.$_POST['bblm_spp'.$p].'\'';
			if ($_POST['mng'.$p])  {
...
<?php
$playersql = 'SELECT P.*, M.mp_inj, M.mp_inc, T.t_name FROM bb_match_player M, bb_player P, bb_team T WHERE P.p_id = M.p_id AND m_id = '.$_POST['bblm_mid'].' AND M.t_id = T.t_id ORDER BY P.t_id, P.p_num';
...
?>
	...
	<?php
//$playersql = "SELECT P.p_id, P.t_id, P.p_name, P.p_num, T.t_name from bb_player P, bb_team T where P.t_id = T.t_id AND (P.t_id = ".$tAid." OR P.t_id = ".$tBid.") AND P.p_status = 1 AND P.p_mng = 0 ORDER BY P.t_id, P.p_num";
$playersql = 'SELECT P.p_id, P.t_id, P.p_spp, X.post_title AS p_name, P.p_num, T.t_name from bb_player P, bb_team T, bb2wp J, '.$wpdb->posts.' X where P.p_id = J.tid AND J.prefix = \'p_\' AND J.pid = X.ID AND P.t_id = T.t_id AND (P.t_id = '.$tAid.' OR P.t_id = '.$tBid.') AND P.p_status = 1 AND P.p_mng = 0 ORDER BY P.t_id, P.p_num';
	if ($playerlist = $wpdb->get_results($playersql)) {
		//initiate var for count
		$p = 1;
		$is_first = 1;
...
?>
	...
	<?php
$matchsql = "SELECT M.m_id, M.m_date, T.t_name AS tA, Q.t_name AS tB, M.m_teamAtd, M.m_teamBtd, M.m_gate, P.guid, C.c_name FROM bb_match M, bb2wp J, ".$wpdb->posts." P, bb_team T, bb_team Q, bb_comp C WHERE C.c_id = M.c_id AND M.m_id = J.tid AND J.pid = P.ID AND J.prefix = 'm_' AND M.m_teamA = T.t_id AND M.m_teamB = Q.t_id AND M.m_complete = 0 ORDER BY m_date DESC, m_id DESC";
