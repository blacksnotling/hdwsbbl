...
<?php
		 /*-- Stats part A -- */
		 $mostxteamseasonsql = 'SELECT A.ats_value AS VALUE, T.WPID, A.sea_id FROM '.$wpdb->prefix.'awards_team_sea A, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' S WHERE A.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 12 ORDER BY VALUE DESC, A.sea_id ASC LIMIT 1';
		 ...
		 $mostxteamcompsql = 'SELECT A.atc_value AS VALUE, T.WPID, A.c_id AS CWPID FROM '.$wpdb->prefix.'awards_team_comp A, '.$wpdb->prefix.'team T, WHERE A.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 12 ORDER BY VALUE DESC, A.c_id ASC LIMIT 1';
...
<?php
				...
				if ($topstats = $wpdb->get_results($statsql)) {
				 ...
				 // Most Vicious Teams //
				////////////////////////
				$statsql = 'SELECT Z.WPID, SUM(T.tc_casfor) AS VALUE, R.r_name, Z.t_active FROM '.$wpdb->prefix.'team_comp T, '.$wpdb->prefix.'comp C, '.$wpdb->prefix.'team Z, '.$wpdb->prefix.'race R WHERE R.r_id = Z.r_id AND Z.t_id = T.t_id AND Z.t_show = 1 AND C.c_id = T.c_id AND C.c_counts = 1 AND Z.type_id = 1 '.$statsqlmodt.'GROUP BY T.t_id ORDER BY VALUE DESC LIMIT '.$stat_limit;
