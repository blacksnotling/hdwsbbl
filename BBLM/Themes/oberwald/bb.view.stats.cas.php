...
<?php
		 /*-- Stats part A -- */
		 $mostxteamseasonsql = 'SELECT A.ats_value AS VALUE, T.WPID, S.post_title, S.guid FROM '.$wpdb->prefix.'awards_team_sea A, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' S WHERE A.sea_id = J.tid AND J.prefix = \'sea_\' AND J.pid = S.ID AND A.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 12 ORDER BY VALUE DESC, A.sea_id ASC LIMIT 1';
		 $mxts = $wpdb->get_row($mostxteamseasonsql);
		 $mostxplayerseasonsql = 'SELECT A.aps_value AS VALUE, L.post_title AS PLAYER, L.guid AS PLAYERLink, T.WPID, S.post_title, S.guid, X.pos_name FROM '.$wpdb->prefix.'awards_player_sea A, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' S, '.$wpdb->prefix.'player P, '.$wpdb->prefix.'bb2wp K, '.$wpdb->posts.' L, '.$wpdb->prefix.'position X WHERE P.pos_id = X.pos_id AND P.p_id = K.tid AND K.prefix = \'p_\' AND K.pid = L.ID AND A.sea_id = J.tid AND J.prefix = \'sea_\' AND J.pid = S.ID AND A.p_id = P.p_id AND P.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 12 ORDER BY VALUE DESC, A.sea_id ASC LIMIT 1';
		 $mxps = $wpdb->get_row($mostxplayerseasonsql);
		 $mostxteamcompsql = 'SELECT A.atc_value AS VALUE, T.WPID, S.post_title, S.guid FROM '.$wpdb->prefix.'awards_team_comp A, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' S WHERE A.c_id = J.tid AND J.prefix = \'c_\' AND J.pid = S.ID AND A.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 12 ORDER BY VALUE DESC, A.c_id ASC LIMIT 1';
...
?>
		<ul>
			<li><strong>Most Casualties caused in a Season (Team)</strong>: <?php print($mxts->VALUE); ?> (<a href="<?php print( get_post_permalink( $mxts->WPID ) ); ?>" title="Learn more about this Team"><?php print( esc_html( get_the_title( $mxts->WPID ) ) ); ?></a> - <a href="<?php print($mxts->guid); ?>" title="Read more on this Season"><?php print($mxts->post_title); ?></a>)</li>
			<li><strong>Most Casualties caused in a Season (Player)</strong>: <?php print($mxps->VALUE); ?> (<a href="<?php print($mxps->PLAYERLink); ?>" title="See more on this Player"><?php print($mxps->PLAYER); ?></a> - <?php print( esc_html( $mxps->pos_name ) ); ?> for <a href="<?php print( get_post_permalink( $mxps->WPID ) ); ?>" title="Learn more about this Team"><?php print( esc_html( get_the_title( $mxps->WPID ) ) ); ?></a> - <a href="<?php print($mxps->guid); ?>" title="Read more on this Season"><?php print($mxps->post_title); ?></a>)</li>
			<li><strong>Most Casualties caused in a Competition (Team)</strong>: <?php print($mxtc->VALUE); ?> (<a href="<?php print( get_post_permalink( $mxtc->WPID ) ); ?>" title="Learn more about this Team"><?php print( esc_html( get_the_title( $mxtc->WPID ) ) ); ?></a> - <a href="<?php print($mxtc->guid); ?>" title="Read more on this Competition"><?php print($mxtc->post_title); ?></a>)</li>
...
<?php
				...
				if ($topstats = $wpdb->get_results($statsql)) {
				 ...
				 // Top Killing Teams //
				///////////////////////
				$statsql = 'SELECT COUNT(*) AS VALUE , T.WPID, T.t_active, R.r_name FROM `'.$wpdb->prefix.'player_fate` F, '.$wpdb->prefix.'player P, '.$wpdb->prefix.'match M, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'comp C, '.$wpdb->prefix.'race R WHERE T.r_id = R.r_id AND P.t_id = T.t_id AND (F.f_id = 1 OR F.f_id = 6 OR F.f_if = 7) AND P.p_id = F.pf_killer AND F.m_id = M.m_id AND M.c_id = C.c_id AND C.type_id = 1 AND C.c_counts = 1 AND C.c_show = 1 '.$statsqlmodt2.'GROUP BY T.t_id ORDER BY VALUE DESC, T.t_id ASC LIMIT '.$stat_limit;
