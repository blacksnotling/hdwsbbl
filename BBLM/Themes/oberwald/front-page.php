...
<?php endwhile; ?>
	...
	<?php
					$matchsql = 'SELECT M.m_gate, M.m_teamAtd, M.m_teamBtd, P.guid, P.post_title, C.WPID AS CWPID FROM '.$wpdb->prefix.'match M, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' P, '.$wpdb->prefix.'comp C WHERE M.c_id = C.WPID AND C.c_counts = 1 AND C.type_id = 1 AND M.m_id = J.tid AND J.prefix = \'m_\' AND J.pid = P.ID ORDER BY M.m_date DESC LIMIT 6';
