...
<?php
		 /*-- Stats part A -- */
		 $mostxteamseasonsql = 'SELECT A.ats_value AS VALUE, T.WPID, S.post_title, S.guid FROM '.$wpdb->prefix.'awards_team_sea A, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' S WHERE A.sea_id = J.tid AND J.prefix = \'sea_\' AND J.pid = S.ID AND A.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 11 ORDER BY VALUE DESC, A.sea_id ASC LIMIT 1';
		 ...
		 $mostxteamcompsql = 'SELECT A.atc_value AS VALUE, T.WPID, S.post_title, S.guid FROM '.$wpdb->prefix.'awards_team_comp A, '.$wpdb->prefix.'team T, '.$wpdb->prefix.'bb2wp J, '.$wpdb->posts.' S WHERE A.c_id = J.tid AND J.prefix = \'c_\' AND J.pid = S.ID AND A.t_id = T.t_id AND T.type_id = 1 AND A.a_id = 11 ORDER BY VALUE DESC, A.c_id ASC LIMIT 1';
...
?>
		<ul>
			<li><strong>Most Touchdowns scored in a Season (Team)</strong>: <?php print($mxts->VALUE); ?> (<a href="<?php print( get_post_permalink( $mxts->WPID ) ); ?>" title="Learn more about this Team"><?php print( esc_html( get_the_title( $mxts->WPID ) ) ); ?></a> - <a href="<?php print($mxts->guid); ?>" title="Read more on this Season"><?php print($mxts->post_title); ?></a>)</li>
			<li><strong>Most Touchdowns scored in a Season (Player)</strong>: <?php print($mxps->VALUE); ?> (<a href="<?php print($mxps->PLAYERLink); ?>" title="See more on this Player"><?php print($mxps->PLAYER); ?></a> - <?php print( esc_html( $mxps->pos_name ) ); ?> for <a href="<?php print( get_post_permalink( $mxps->WPID ) ); ?>" title="Learn more about this Team"><?php print( esc_html( get_the_title( $mxps->WPID ) ) ); ?></a> - <a href="<?php print($mxps->guid); ?>" title="Read more on this Season"><?php print($mxps->post_title); ?></a>)</li>
			<li><strong>Most Touchdowns scored in a Competition (Team)</strong>: <?php print($mxtc->VALUE); ?> (<a href="<?php print( get_post_permalink( $mxtc->WPID ) ); ?>" title="Learn more about this Team"><?php print( esc_html( get_the_title( $mxtc->WPID ) ) ); ?></a> - <a href="<?php print($mxtc->guid); ?>" title="Read more on this Competition"><?php print($mxtc->post_title); ?></a>)</li>
