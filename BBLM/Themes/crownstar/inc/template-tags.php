...
if ( ! function_exists( 'crownstar_header_widgets' ) ) :
/**
 * Display header widgets section.
 */
function crownstar_header_widgets() {
...
if ( ! function_exists( 'crownstar_header_branding' ) ) :
/**
...
if ( ! function_exists( 'crownstar_header_banner' ) ) :
/**
 * Display header banner section.
 */
function crownstar_header_banner() {
	?>
...
if ( ! function_exists( 'crownstar_header_area' ) ) :
/**
 * Display header area sections.
 */
function crownstar_header_area() {
	$options = get_option( 'crownstar', array() );
	...
	$sections = apply_filters( 'crownstar_header_area_sections', array(
		'widgets',
		'branding',
		'banner',
		'menu',
	...
	<?php } ?>
		<?php do_action( 'crownstar_before_header_area', $options, $sections ); ?>
		<div class="header-inner">
			<?php
			foreach ( $sections as $section ) {
				if ( 'widgets' == $section ) {
					crownstar_header_widgets();
				} elseif ( 'banner' == $section && $options['header'] && 'image' == $options['style'] ) {
					crownstar_header_banner();
		...
		<?php do_action( 'crownstar_after_header_area', $options, $sections ); ?>
	</div><!-- .header-area -->
	<?php
}
endif;

if ( ! function_exists( 'crownstar_paging_nav' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 */
function crownstar_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'crownstar' ); ?></h1>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'crownstar' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'crownstar' ) ); ?></div>
			<?php endif; ?>
...
if ( ! function_exists( 'crownstar_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 */
function crownstar_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'crownstar' ); ?></h1>
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;%title', 'Previous post link', 'crownstar' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'crownstar' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
if ( ! function_exists( 'crownstar_entry_title' ) ) :
/**
 * Prints HTML with title and author.
 */
function crownstar_entry_title() {
	the_title( '<h1 class="entry-title single-entry-title">', '</h1>' );
}
endif;

if ( ! function_exists( 'crownstar_entry_date' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time.
 */
function crownstar_entry_date() {
	$options = get_option( 'crownstar', array() );
...
if ( ! function_exists( 'crownstar_entry_meta' ) ) :
/**
...
if ( ! function_exists( 'crownstar_entry_footer' ) ) :
/**
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( __( 'Category: %s', 'crownstar' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( __( 'Tag: %s', 'crownstar' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( __( 'Author: %s', 'crownstar' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		$title = sprintf( __( 'Year: %s', 'crownstar' ), get_the_date( _x( 'Y', 'yearly archives date format', 'crownstar' ) ) );
	} elseif ( is_month() ) {
		$title = sprintf( __( 'Month: %s', 'crownstar' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'crownstar' ) ) );
	} elseif ( is_day() ) {
		$title = sprintf( __( 'Day: %s', 'crownstar' ), get_the_date( _x( 'F j, Y', 'daily archives date format', 'crownstar' ) ) );
	} elseif ( is_tax( 'post_format', 'post-format-aside' ) ) {
		$title = _x( 'Asides', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
		$title = _x( 'Galleries', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-image' ) ) {
		$title = _x( 'Images', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
		$title = _x( 'Videos', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
		$title = _x( 'Quotes', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-link' ) ) {
		$title = _x( 'Links', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-status' ) ) {
		$title = _x( 'Statuses', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
		$title = _x( 'Audio', 'post format archive title', 'crownstar' );
	} elseif ( is_tax( 'post_format', 'post-format-chat' ) ) {
		$title = _x( 'Chats', 'post format archive title', 'crownstar' );
	} elseif ( is_post_type_archive() ) {
		$title = sprintf( __( 'Archives: %s', 'crownstar' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
		$tax = get_taxonomy( get_queried_object()->taxonomy );
		/* translators: 1: Taxonomy singular name, 2: Current taxonomy term */
		$title = sprintf( __( '%1$s: %2$s', 'crownstar' ), $tax->labels->singular_name, single_term_title( '', false ) );
	} else {
		$title = __( 'Archives', 'crownstar' );
	}
